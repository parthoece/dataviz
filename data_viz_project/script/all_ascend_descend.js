...
const updateChart = (filePath) => {
  d3.csv(filePath).then((data) => {
    ...
    const aggregatedData = d3
      .rollups(
        filteredData,
        (v) => d3.sum(v, (d) => d["Refugees under UNHCR's mandate"]),
        (d) => d[groupBy]
      )
    ...
    const chartHeight =
      aggregatedData.length * (stackedBarHeight + stackedBarSpacing);
    stackedSvg.attr(
      "height",
      chartHeight + stackedMargin.top + stackedMargin.bottom
    );

    yScale.domain(aggregatedData.map((d) => d.country)).range([0, chartHeight]);
    xScale.domain([0, d3.max(aggregatedData, (d) => d.value)]);
    ...
    const bars = chart.selectAll(".bar").data(aggregatedData, (d) => d.country);
    ...
    bars
      ...
      .on("mouseover", function (event, d) {
        d3.select(this).attr("fill", "#732E00");
        tooltip
          .style("opacity", 1)
          .html(
            `<b>${d.country}</b><br>Refugees under UNHCR's mandate: ${d.value}`
          )
      ...
      .on("mouseout", function () {
        d3.select(this).attr("fill", "#CA6C0F");
...
//     const bars = chart.selectAll(".bar-group").data(top10Data, (d) => d.country);
...
//     const stackedBars = barsEnter
//       .selectAll("rect")
//       .data((d) => d.years, (d) => d.year);
...
//           .slice(0, i)
//           .reduce(
//             (sum, node) => sum + xScale(node.__data__.value),
//             0
//           );
